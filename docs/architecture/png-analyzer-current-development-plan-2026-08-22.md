...
> Baseline commit: `8ec6b83` (`main`，WP-605A portable package smoke 完成后的代码基线)
...
| M6 Validation、Statistics、发布 | WP-600A～WP-600C、WP-603A～WP-603C、WP-604A～WP-604B、WP-605A 已实现 | Structural/integrity/semantic/decode/resource 规则、单一 analysis-engine 聚合入口、CLI JSON 与 GUI worker 状态整合、固定 fuzz replay 的 ASan/UBSan 门禁、生成式性能 corpus/runner、机器记录 schema、固定阈值 gate 与 portable package smoke | Statistics、coverage-guided fuzz、跨平台性能基线、WP-605B 文档、RC 审计 |
| M7 APNG | 未开始 | 模型预留 frame 维度 | 维持 post-v1 |

截至当前提交，M0～M4、M5 的 WP-500～504、WP-5U0～WP-5U6C、WP-5T0A～WP-5T0B 及 M6 的 WP-600A～WP-600C、WP-603A～WP-603C、WP-604A～WP-604B、WP-605A 已有实现提交。这里的“实现完成”不等于里程碑 Gate 已关闭；Gate 仍要求相应 coverage-guided corpus、跨平台性能、原生安装器和人工交互证据。
...
- 本次核验覆盖当前 `main`；WP-605A 的 CPack、package smoke、CI 与计划文档变更在验证后统一提交。

本次未声称已通过：正式 conformance/coverage-guided fuzz、跨平台 performance threshold、原生 DMG/MSIX/AppImage/Flatpak 或 Qt framework deployment。dev 与 ASan/UBSan 全量、WP-603C 定向 replay、WP-604A runner、WP-604B threshold gate、WP-605A portable archive smoke 与 GUI performance scenario 均已通过；原生安装器和发布证据仍属后续 Gate。
...
当前下一项是 `WP-605B User & Developer Docs`：补齐 README、使用手册、trace
语义、贡献与 bug report 流程；不得在 GUI 重写 Deflate 解析。
