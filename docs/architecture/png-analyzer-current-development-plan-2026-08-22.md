...
> Baseline commit: `066f85b` (`main`，WP-505B 完成后的基线)
...
| M5 Deep Deflate Trace | Block Inspector 与 Huffman Tables 已实现，Trace 编排与 UI 主线已实现 | WP-500～504、WP-5U0～WP-5U5B、WP-5T0A～WP-5T0B、WP-505A～WP-505B | WP-505C、端到端 Trace Gate、fuzz/sanitizer 仍未完成 |
...
- 26/26 个 CTest 测试入口通过，包含 core、parser、reconstruction、Deflate、differential、CLI 与 GUI 测试；新增 Block/Huffman Inspector 的 Qt-free 与 Qt 测试。
...
- WP-500～504 已提供 wrapper、block、Huffman、token、LZ source 和 pixel provenance 基础；WP-5T0A～WP-5T0B 已形成受预算、可取消、generation 安全的聚合查询链路；WP-505A～WP-505B 已将 block 关联范围、码表构建顺序和导航上下文投影到 Qt-free/UI。
...
WP-5U1～WP-5U5B、WP-5T0A～WP-5T0B、WP-505A～WP-505B 已落地。当前下一项是
`WP-505C Decode Trace`：把 literal/match 的逐步计算、overlap source 和当前
output byte 接入同一 Qt-free/UI 查询边界；不得在 GUI 重写 Deflate 解析。
