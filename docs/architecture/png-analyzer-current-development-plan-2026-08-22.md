...
> Baseline commit: `f3fb5dd` (`main`，WP-603A fuzz smoke 完成后的代码基线)
...
| M6 Validation、Statistics、发布 | WP-600A～WP-600C 已实现 | Structural/integrity/semantic/decode/resource 规则、单一 analysis-engine 聚合入口、CLI JSON 与 GUI worker 状态整合 | Statistics、fuzz 回归/coverage、发布证据 |
...
WP-5U6A～WP-5U6C、WP-600A～WP-600C、WP-603A 已落地。当前下一项是
`WP-603B Decode/Reconstruction Fuzz`：为 Deflate trace、filter、Adam7 与
packed samples 建立同样受限、可重放的 harness；不得在 GUI 重写 Deflate 解析。
