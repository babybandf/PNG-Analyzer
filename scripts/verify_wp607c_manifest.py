...
def _check_catalog_record(prefix, record, catalog):
    if not isinstance(catalog, dict):
        return [f"{prefix}: catalog must be a mapping"]
    case_id = record["id"]
...
def run_self_test():
    ...
    good_catalog = {
        record["id"]: {key: record[key] for key in sorted(GENERATED_KEYS)}
        for record in good if record["kind"] == "generated"
    }
    ...
    expect_reject("catalog entry missing", good, "missing from the catalog",
                  catalog={})
    drifted_catalog = dict(good_catalog)
    drifted_catalog["ui-gray1-none"] = dict(
        good_catalog["ui-gray1-none"],
...
def refresh_generated_hashes(manifest_path, catalog):
    ...
    byte-identical; this function trusts `catalog` for the new values.
    """
    ...
    for index, line in enumerate(lines):
        ...
        if hash_match and current_id in catalog:
